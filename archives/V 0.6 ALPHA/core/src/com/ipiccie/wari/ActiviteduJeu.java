...
public class ActiviteduJeu extends ApplicationAdapter {
	private static final float GRAVITE = -10;
	private static final float LATERAL_GRAVITE = 10;
	private static final float VAL_IMPULSION_SAUT = 350;
	private static final float VITESSE_MAX_X = 100;
	private static final float VITESSE_MAX_Y = 300;
	...
	public void create () {
		...
		laCarte = new TmxMapLoader().load("carte2.tmx");
		...
		Perso.HEIGHT = (float)(regions[0].getRegionHeight()/10)*echelle;
		Perso.WIDTH = (float) (regions[0].getRegionWidth()/10)*echelle;
		...
		camera.setToOrtho(false,20*echelle*(largeur / hauteur),20*echelle);
		leGrandReset();
	}
	...
	public float collision(float deltaTime){
		...
		position.y-= 2;
		Perso.HEIGHT += 4;
		persoRect.set((position.x+10)/echelle,position.y/echelle,(Perso.WIDTH - 10)/echelle,Perso.HEIGHT/echelle);
		...
		tuiles = obtientTuiles(startX,startY,endX,endY,tuiles, "obstacles");
		...
		Gdx.app.log("infos perso","ok"+personnage.position.y+ " " +position.y);
		tuiles = obtientTuiles(startX,startY,endX,endY,tuiles, "sol");
		for (Rectangle tuile : tuiles){
			if (persoRect.overlaps(tuile)){
	...
	public Array<Rectangle> obtientTuiles(int startX, int startY, int endX, int endY, Array<Rectangle> tiles, String calque){
